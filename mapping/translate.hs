...
bigTest = main' "testShortSimple.xml" >>= writeFile "mappingShort.txt" . unlines
...
pComplV2 = do
  ...
  adv <- maybeVerbAdv
  obj <- liftM (Just .fst) (inside "OO" pNP)
         `mplus`
         liftM (Just .fst) (inside "SP" pNP)
         `mplus`
         do o <- inside "OA" (cat "PP" `mplus` cat "VP")
            return (Just $ mkApp meta [o]) --tveksamt fall?
         `mplus`  
