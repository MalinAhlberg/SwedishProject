...
-- proper plural and comparison forms "galna-galnare-galnast".
    ...
    --mkA : (peppad,peppat,peppade : Str) -> A ; -- irregular comparison
                                               -- but no compartive is required
...
-- can also be used for irregular verbs without compareative forms
  compoundA : overload {
  
   compoundA : A -> A ;
   compoundA : (peppad,peppat,peppade: A) -> A
...
-- School declensions.
  ...
  mkA = overload {
    ...
    -- how should this be? force compounA seems dangerous..
--    mkA : (peppad,peppat,peppade : Str) -> A = 
--     \peppad, peppat, peppade ->
--       compoundA (mk7A peppad peppat peppade peppade peppad+"are" peppad+"ast" peppad) ;
 ...
 -- should not need to force isComp to be True, but might as well.
  compoundA = overload {
     compoundA : A -> A = mkComp ;
     compoundA : (peppad,peppat,peppade : Str) -> A = mkCompA
     };

  mkCompA : (_,_,_ : Str) -> A ;
  mkCompA peppad peppat peppade = 
   let peppa = init peppad in 
    -- give normal comparatives, will not be used anyway
   mkComp (mkA peppad peppat peppade (peppa+"re") (peppa+"st")); 
  mkComp : A -> A ;
  mkComp adj = lin A {s = adj.s ; isComp = True ; lock_A = <>} ;
