...
multi_printer = concat . nub . map prOne . multi
 where prOne (x,stem,cat,inhs,id,para,attr) = conc [p "word" x, p "head" stem, p "pos" cat, p "param" "frag", pl "inhs" inhs, p "id" id, p "p" para, p "attr" attr]
       pl s []   = quote s ++ ":" ++ "[]"
       pl s xs   = quote s ++ ":[" ++ (concat (intersperse "," (map quote xs))) ++"]"
       p s1 s2 = quote s1 ++ ":" ++ quote s2
       conc xs = '{':(concat (intersperse "," xs)) ++ "}\n"

multi = concat . map prOne . filter is_multi. unDict
 where prOne  (id,stem,para,cat,inhs,tbl,_) = 
           [(x,stem, cat, inhs, id, para, (show attr)) |  (a,(attr,s)) <- (merge tbl), str <- unStr s, length (words str) > 1, x <- map unwords $ gen (words str)]
       merge            [] = []
       merge           [x] = [x]
...
prFlatXML d =  unlines [
  ...
  "</Lexicon>"]
 where
 pr e@(id,stem,para,cat,inhs,tbl,extr) =  
     concat 
         [
          "   <LexicalEntry>\n",
          "    <lem>" ++ id ++ "</lem>\n",
