...
{The opinions& that& put+{\sc s} & forward ...}
...
\enumsentence{Somligas skor står kvar i hallen\\
...
%notice if the grammar starts to overgenerate. Avoiding ambiguities is 
...
\begin{itemize}
\item one large-scale GF lexicon and a program to redo the importation when needed
...
The increase of the grammatical coverage is hard to compare, since
The grammar in it self is not at all robust, and one unknown word will cause a whole sentence to 
fail. 
An estimation can be given by looking at the results from running the test suite.
The first grammar could parse almost halv of the sentences, and the final
grammar could parse 65\%. 
When parsing text, we get low results, for this reason. Not yet interesting to talk about
...
No syntactic theory will ever cover all language, and this should just be seen as an important
fragment.


From the renewed import of Saldo, we got a much better coverage especially considering
that the old version was missing many of the most common words. Bo, göra, se.
However, the lexicon is now to big to use as a normal GF lexicon, since it will request
too much memory to parse most sentences. We will need to handle this in a clever way,
see future work.
...
\section{Future Work}
\label{sec:future}
A part of the project is finished, but the project is not complete, and will never be.
Many interesting future directions. We will first look at the ones which are 
direct consequences of what we did so far. 
...
a. & jag & ser&  honom&  inte&~~ & b.&   * jag &ser &huset     &inte\\
   & I   & see&  him  &  not & & b.&    I   &see &the house &not\\
\end{tabular}}
