...
constructions. We have completed a method for importing the electronic 
...
would also mean that already existing resources were be reimplemented.
...
implementation and some results. Finally, the conclusion and evaluation are
...
An intercative GF editor and the GF shell can be tried out online 
...
The project talk \cite{talk} used GF to describe dialogue grammars and
...
different languages and there are now grammars for English, German, Japanese and Norweigan. 
...
An important part of this project has been to develope the Swedish GF grammar and
...
usually a record containing a table with the forms the phrase can be used in.
They may also
carry information about inherent features that matter to other phrases.
...
and field keeping information about the gender.
\begin{verbatim}
...
\verb-NP- by setting the number and definitness of a \verb-CN-.
...
\begin{verbatim}
DetCN : Det                    -> CN     -> NP ;
        indefinitive, singular -> flicka -> "en flicka"
...
in our grammar must keep information of which definitness they allow, the
\verb-DetSpecies- parameter. The number is also stored as an inherent 
feature of the determiner.
The resource grammar differenciates between quantifiers, 
...
is the either \emph{``de"} or nothing, depending on whether the noun phrase
has been modified.
This information must thus be stored in the \verb-NP-. 
\begin{figure}[h]
\begin{verbatim}
DetNP : Det              -> CN        -> NP ;
...
Many words which are considered to be pronouns in other analyses are 
classified as quantifiers and determiners in GF.
...
---------
When a category has been formed, there is no longer any information available
about how it was put together. This is a result of the functional approach
of GF. In the case shown in figure \ref{gfcode:defmod},
we would like to know whether
the it has been modified or not.
%that is, whether the function \verb|AdjCN| has been applied.
This information
has to be passed on by an inherent feature, a flag set to tell
...
The funciton \verb-SlashV2a- lets you create a \verb-VPSlash-, a verb phrase missing a
noun phrase \cite{gazdar}. We combine the \verb-SlashVP- with the object and
...
it has multiple fields that may be put in the different orders. There are fields
for negation, different types of adverbs, objects, the finite verb and an infinite verb.
The fields may in turn be tables showing the differnt forms of the component.\\

\begin{tabular}{|l|lllllll|}
\hline
\textbf{Subj} && &\textbf{VP}& & & & \\
    & \sc finit& \sc neg& \sc adV & \sc inf& \sc comp& \sc obj& \sc adv \\
\hline
...
\vspace{5mm}
The fields form the verb phrase are not put in the correct order until a sentence is
created.\\
...
pronoun and \verb-partV- gives the particle.
The fact that the choosen prepositions is attatched to the verb in the lexicon
causes the parse tree visualisation algorithm group them together.
This is also the case for particles, cf. parse tree \ref{pic:tittapart}
and \ref{pic:tittapa}.
As already stated, the visualized parse trees is not a complete represtation,
...
not fronting of particles as in\ref{sent:tittapa}b.
\eenumsentence{
...
\hline
Presens &Negative & Main        & du ser mig & 'Du ser inte mig'\\
...
\hline
\textbf{Wh-Questions} & Interrogativ pronoun   & Vad tittar Johan på?\\
...
\subsection{Overview}
\vspace{4mm}
The resource grammar could express complex sentences
...
The resource grammar covered relative clauses: 
...
In addition to the core of the resource grammars, which is shared with the other languages implemented in the
library, 
...
When first conducting tests in this project, we found much room for improvement.
From the topcis listed in section \ref{sec:swedish}, the post-nominal artciles
...
During this project, the s-passive was added as the standard case for the Swedish GF
...
\enumsentence{
*Det sitter min fågel på taket\\
``There are my bird sitting on the roof''\\
}\label{sent:dumfagel}
The destinction lays in the determiner. The different types of determiners are
showed in figure \ref{tab:detdef}. 
For real subjects, the determiner must be of type \verb-DIndef-,
that requires both the noun and the adjective are used in indefinite form.
...
If the determiner does not fulfill the requriments stated above, the clause is
put to NONEXIST.
...
type level, it cannot be known until runtime whether the determiner is accepted
in the subject.

% move to future?
As a future direction it would be intresting to examine the consequences of
...
needs an antecendant.
We would also like to differenciate between the \verb-NP-s in sentence
...
He wants me to look at {\sc self}.}
...
\item The possiblity for noun phrases to agreemee with an antecedent 
...
a. & Det \textbf{kommer att} bli mörkt snart & & b. &Jag \textbf{ska} gå och lägga mig nu\\
  & ``It will get dark soon" & & b. &``I'm going to bed now"\\
\end{tabular}}
\enumsentence{
...
a. & Du ska tycka om den & & b. &Du kommer tycka om den\\
  & ``You \emph{shall} like it" & & b. &``You will like it."\\
\end{tabular}}
...
{it & is & only & childern &that& may & do & so }
...
All construction used in examples \ref{sent:relex}a-d was accepted.
...
As a paranthesis, some complications regarding the function \verb-RelCl- might
be pointed out.
...
and the Swedish version \emph{`sådan att'} sounds awkward, when used outside of
logic books.
\enumsentence{Jag vill ha en katt sådan att den inte fäller.} 
This relative clause \emph{``en katt sådan att den inte fäller"} is, in GF, put
together by two constituents: \emph{``katt"}
...
agreement. However, it shoud agree with its antecedent, \emph{`katt'}. 
\enumsentence{*Jag vill ha en katt sådan att \textbf{det} inte fäller.} \label{sent:detfaller}
The dependent types introduced in section \ref{sec:reflexives} could probaly be
useful.
However a carfully prepared analysis and motiv are needed. %ationamount in a bigger reconstruction which therefore
%would require a. 
Consequentelly, sentence \ref{sent:detfaller} is still
...
\begin{itemize}
\item Quantifiers does not have any possessive
forms, so when a quantifier is turned into
a NP (by the functions \verb-DetQuant- and \verb-DetNP-), its genitive field gets
the same form as its nomitative field.
\item\verb-GenNP-
introduced a cycle: a \verb-Quant- may be turned into a \verb-NP-, and now also
a \verb-NP- can be turned into a Quant.
\end{itemize}
\begin{verbatim}
...
and consequentelly as a noun phrase:
...
\verb-DetNP-, that is responsible for creating the possessive form
...
the best approximation to what it would look like, given that it indeed was used in 
Swedish. Most important is however that this stopped the \verb-GenNP-
from introducing unnecessarily ambiguities.
The number of parse trees for the phrase \emph{``Kattens bils hus"} was reducesed
from over one hundered to six.
...
information from Talbanken have conducted. We were interested in words which can be used
...
By starting from a list containing all pronouns in Saldo and all forms they may occure in,
...
This method is
both faster and more reliable than going through the list by hand and categories them
by manually looking up each word or by using intraspection.
...
would notice if new ambiguites or overgenerating functions were introduced.
...
class as another tag are excluded, Talbanken still leaves us with more than 130 tags. The 
...
of covering a whole natural language, our grammar when used without a
robust layer should just be seen as an important
fragment.\\

A comparison with Talbanken would not be of much value of this state since the coverage 
is yet too small to do be able to get any interesting analysis.
...
directions. As a natural language processing-project ~ it will hardely ever be entirely
complete.
...
Some work must be left for the robustness, but some specific constructions
are desirable and interesting to implement. A few examples are listed here:\\

In many Germanic? languages \textit{object shifts} are common.
...
\end{tabular}}
Personal pronouns are allowed to shift position with the negation in main clauses.
(read).
\enumsentence{\begin{tabular}[t]{@{}*{11}{l@{\ }}}
...
This is common in relaxed? language, but since it is rarely found in
Talbanken's part P, which we have been using, it has not been highly prioritised.\\
...
Nouns used without any explicit definitness can at this stage be parsed as mass
nouns.
\enumsentence{\shortexnt{3}
{Jag & dricker& vatten}
{\emph{I}&\emph{drink}&\emph{water} }
}
This is not the correct analyse for `hund' in sentence \ref{sent:hund}.
\enumsentence{\shortex{4}
{Vi & ska& köpa & hund}
{\emph{we} &\emph{will}& \emph{buy} & \emph{dog} }
{We are going to buy a dog}}\label{sent:hund}
This sentence is instead an example of \textit{bare indefinites}, used for
... what?\\
...
The multipleword verbs (\verb-vbm-) and multiple word nouns (\verb-nm-) should
be imported to an idiom lexicon, which can be used as extensions to the main
lexicon.
For the words that we tried but failed to import, a tool for lexicon
...
compounds words and unknown constructions.
\subsubsection{Parallel parsing/Chunk parsing}
Instead of parsing a whole sentence in one go, we would like to
parallely parse parts of it and then try to combine the pieces.
This method should be faster then traditional parsing and would
at the same time give robustness.
Components that cannot be combined by any grammar rule are combined by a 
meta function. We might end up with partial parse trees, but we would
...
compounding analyses.

We need to come up with better methods for using the lexicon when
parsing, since its time and memory consumption is very high.
One possibility is of course to do 
deeper refinements of the parsing source code, which would be a substantial 
project. Other solutions are to to either
adapt the lexicon for its domain and hence making it smaller and faster
or to adapt the input sentences to a smaller lexicon, by preprocessing .

Further, the lexicon needs valencies since this information lays the
foundation for the GF analysis. We have already analysed how Talbanken
annotates valency information, and by using the rules made for the mapping
described in section \ref{sec:Mapping}, we could extract data from Talbanken,
showing how words are normally used. There should also be algorithms for
automatically adding the information to the lexicons. 
Many Swedish verbs can be used with different number of arguments. Adding
all this information to the lexicon does not seem to be a good idea considering
...
The grammar will always contain ambiguities but before returing a parse result
to the user, some analysis should be done to find the most likely tree.
When the size of the lexicon
increases, so will the ambiguities as the number of word forms overlapping each other
gets higher. Our new grammar also have many more rules which also will contribute
to increase the number to interpret each sentence.

GF already allows the lexicon to have probabilities, and by using our extracted
GF treebank, we can get reliable statisics for our functions. 
...
for a deep Swedish parser. %To abstract:  by combining resources.
Our main resource is the grammar formalism GF,
which allows us to start from a well-defined system for describing grammar.
The parser is derived from the grammar and will therefore validate that
a given sentence is grammatically correct according to the rules defined.
All parts, including the underlying grammar, are open-source and 
may thus be used in applications dealing with controlled natural languages.

Even though the grammar will not be totally covering, we belive that
the work done is both interesting and useful.
