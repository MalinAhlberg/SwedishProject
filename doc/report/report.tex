...
language(xx,xx). Unfortunately, it is no longer available. The coverage of the Swedish
...
incremental parsing algorithm use more heap memory than what normal computers
...
directions. As a natural language processing-project it will probably never be
entirely complete.
...
Some work must be left for the robustness, but there are specific constructions
that are desirable and interesting to implement. A few examples are listed here:\\

\begin{itemize}
\item
\textit{Pronomial object shifts} are common in Swedish.
...
Therefore, it has so far not been prioritised.\\
...
\item
\textit{Bare indefinites} are at this stage always parsed as mass nouns.
\enumsentence{\shortexnt{3}
{Jag & dricker& vatten}
{\emph{I}&\emph{drink}&\emph{water} }
}
This however is not the correct analyse for `hund' in sentence \ref{sent:hund}.
...
It should for example be possible to differenciate between 
...
The grammar will always contain ambiguities but before returing the result
...
GF treebank, we can get more reliable statisics for our functions. 
...
may thus be used in other applications.
