...
resource grammars. 
A lexicon normally consist of words from the open word-classes, shown in rectangular boxes in
the picture. Each word has a type describing its word class, and the abstract grammar
consists of rules for how the parts of speech can be used -- how the types relate
to each other.
\begin{figure}[h]
...
A category is represented by a table covering the forms the phrase can be used in.
They may also
...
The \verb-N- category is implemented as an inflectiontable and field keeping information
about the gender.
...
of GF. In the case of the of the \verb-CN- above,
we would like to know whether
the it has been modified or not, that is, whether the rule
\verb|AdjCN : AP -> CN -> CN ;| has been applied. Insted the information
is passed on by an inherent feature, a flag telling set to tell
...
\enumsentence{På pojken tittar du \\ **På när du springer tittar jag}
...
implementation, since there was no big resource to evaluate it on.
...
described above.
There are also restrictions on the real subject.
\enumsentence{\begin{tabular}{ll}
...
\end{tabular}}
Expressed in GF terms, the determiner must have definitness DIndef. That is, all determiners
that requireds the noun and the adjective to be used in indefinite form may be used.
...
allowed as subject. The antecedent must not be outside of the smallest verb phrame??
...
I give {\sc self's} money to {\sc self's}his siblings. }
...
This section covers some minor changes done in the grammar. They are described
...
The grammar has been extended to accept a definite article to be used with a
noun in indefinite form whenever the noun phrase is followed by a restrictive
relative clause.
...
agreement, although it should agree with the antecedent, \emph{`katt'}. 
\enumsentence{*Jag vill ha en katt sådan att \textbf{det} inte fäller.} \label{sent:detfaller}
Changing the type of \verb-NP- would amount in a bigger reconstruction which therefore
would require a carfully prepared analysis and motivation. 
...
\item In combination, \verb-GenNP-
...
\verb-DetNP-, that was responsible for creating the possessive form
of quantifiers and determiners.
Instead we let it append an extra
\emph{s} to the word, so that applying \verb-GenNP- to \emph{`kattens'} results
in \emph{`katten\textbf{ss}'}. Even though this generates form does not exist, it is perhaps
the best approximation to what it would look like, given that it indeed was used in 
Swedish. Most important is however that this changed stopped the \verb-GenNP-
...
When possible, the ambiguities could be removed and when not, we were at least made
