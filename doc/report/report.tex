...
therefore combine already existing parts of ??.
...
stronger than mildly contex-free grammars and can express whatever tree
adjoining grammars (Joshni,xx) and HG?? can.  --very confusing here!!

GF is a strongly typed functional programming languages, inspired by
%programming languages like 
ML \cite{ml} and Haskell \cite{haskell}. Its type
checking system is inspired by \textlambda Prolog \cite{bratko} and the
dependent types by Agda \cite{agda}. 
>> check this!!
The first version of the framework was implemented in 19xx in xx and has
since then fatt en trogen skara användare, med headquarter 
...
The grammars can be compiled into portable grammar format (PGF)\ref{pgf},
...
\begin{figure}[h]
\begin{verbatim} 
> parse "jag ser katten" | visualize_parse
> parse "jag ser katten" | visualize_tree
\end{verbatim}
...
\label{fig:shellvp}
\end{figure}
>> test vp -format=pdf -view=evince
>> test vt -format=pdf -view=evince -nofun
...
\begin{figure}[h]
\centering
\subfloat[GF abstract tree]{\label{pic:gfAtree}\includegraphics[width=60mm]{gfTree.pdf}}
\subfloat[GF parse tree]{\label{pic:gfStree}\includegraphics[width=50mm]{gfPTree.pdf}}
\subfloat[GF parse tree]{\label{pic:gfEtree}\includegraphics[width=50mm]{gfETree.pdf}}
...
The parse tree (\ref{pic:gfStree}) shows the words and which types the functions assign
to them.
...
In the corresponding English parse, figure \ref{pic:gfEtree},
the article \emph{``the"} is explicit and therefore shown in the parse tree.
...
translation. The ambiguities arising from multilinguality and the inherited will of
course remain, but limited. %Example of domainspecific word?
...
may be thought of as fstructures and cstructures in LFG (x,x).
