...
\abstract{
This thesis describes work towards a wide-coverage grammar for
parsing and generating Swedish text.
We do this by using the dependently typed grammar formalism GF,
a functional programming language specialized at describing grammars.
The idea is to 
...
combine existing language resources with
new techniques, with an aim to achieve a parser
for unrestricted Swedish.
To reach this goal, problems of computational as well as linguistic nature had
to be solved.  
The work includes the development of the grammar -- \;
%in the GF formalism
to identify and formalize 
grammatical constructions %specific to and
frequent in Swedish -- as well as methods for importing a large-scale lexicon and for evaluating
the parser. % have been investigated and formalised in the functional
...
We present the methods and technologies used 
and discuss the advantages and problems of using GF
for modeling large-scale grammars.
We further discuss how our long-term goal can be reached
%the possibilities
%of reaching our long-term goal % -- to accomplish a parser for unrestricted Swedish -- 
by combining our rule-based grammar with statistical methods.
%We compare and discuss .
%and for obtaining a system for analysing written text.
...
\vspace{5mm}
Many people have helped me during this project and made this work possible.
...
syntactical structures, is receiving increasing interest, especially
...
\begin{figure}[h]
\centering
\subfloat[Swedish parse tree]{\label{pic:gfStree}\includegraphics[width=30mm]{gfSTreec.pdf}}
...
the abstract tree while the parse tree only shows that the noun phrase consists of one noun. 
...
\noindent The example in figure \ref{fig:gfAbstract1} shows an abstract grammar defining three categories, % \verb|Categories|,
...
\noindent Figure \ref{fig:gfSweCnc1} shows how the abstract grammar can be implemented
...
\noindent We now have two implementations of the abstract, one for Swedish and one for English.
...
\noindent This is the case for `flicka', a noun belonging to the first declination. %the correct declination
...
\noindent The semantic aspect of Saldo requires that words with multiple meanings are
separated into different entries.
%TODO change word? uppskatta, känna, få
The word `uppskatta' for example, has two entries, with slightly
...
\noindent The LinGO Grammar Matrix \cite{matrix}, is a starter-kit for building Head-Driven Phrase
...
\noindent The Swedish version of the Core Language Engine (CLE) \cite{gamback} % \cite{cle}
...
\noindent In the TAG formalism \cite{tag}, there are projects on getting open source, wide-coverage grammars
for English and Korean, but, to our knowledge, not for Swedish.  \\

\noindent The ParGram \cite{pargram} project aims at making wide coverage grammars using
...
\begin{figure}[h]
  \begin{center}
\subfloat[Saldo]{\label{pic:saldoVax}\includegraphics[height=80mm]{saldovax.png}}
\hspace{5mm}
...
\noindent During this project, the program has been made more robust than the previous
...
\noindent Most but not all words of these categories have been imported.
...
\noindent When testing the coverage of Talbanken,
...
\noindent A list of words that were given different labels in GF than in Talbanken has been
...
\begin{figure}[h]
\hspace{-29mm}
\includegraphics[height=130mm]{graph.pdf}
...
\noindent Some rules for determination were described in section \ref{sec:swedishnoun};
...
%TODO review, verify
\noindent Due to the syntax oriented analysis in GF, the GF category for pronouns \verb-PN- only
...
\noindent Swedish verbs may take up to five arguments \cite[p. 53]{stymne}.
...
and \ref{pic:tittapa}.
\begin{figure}[h]
\subfloat[Verb with a chosen preposition]{\label{pic:tittapa}\includegraphics[height=70mm]{tittapac.pdf}}
\hspace{10mm}
\subfloat[Verb with particle]{\label{pic:tittapart}\includegraphics[height=70mm]{tittapartc.pdf}}
...
\end{figure}
\newpage
\noindent As already stated, the visualized parse trees is not a complete representation,
...
\noindent Like verb phrases, a clause may also be missing an object and then has the type
\verb-ClSlash-.
...
\end{figure}
\noindent Even though the verb phrase \emph{``har inte ätit de gula äpplena idag"} is discontinuous, the whole
...
At the code level, this is done using the record type for \verb-VP-, which
...
\noindent During this project, the s-passive was added although the
...
categories. That is,
we know that the first argument of the function \verb-PredVP- acts as the
subject, but
...
all syntactical aspects of Swedish while leaving the abstract grammar
unchanged.
...
Apart from this, only corrections have been done, exemplified in
this sentence.
\enumsentence{
...
It selects the possessive form given in the \verb-NP- and changes its type
to \verb-Quantifier-. 
%\begin{verbatim}
...
\item\verb-GenNP-
introduced a cycle: a \verb-Quant- may be turned into a \verb-NP-, and now
...
As noted in section \ref{sec:gfnouns} our GF grammar only classifies personal pronouns
...
determiner (\ref{ex:QuantDet}), or as an adjective agreeing with the noun
(\ref{ex:QuantAdj}-c). 
\eenumsentence{
\item\shortex{5}
...
{``He's like that"}
\item\shortex{3}
{Huset& är& \label{ex:QuantAdj2}sådan\textbf{t}}
{\emph{the house}& \emph{is}& \label{ex:QuantAdj2}\emph{such}}
...
The implementation consists of a set of rules that translate
nodes of a Talbanken tree to GF functions, possibly by describing what the
...
The meta variable is also used as a function, needed when we have found two
translatable trees but no rule that allows them to be combined.

%Lets look at the example 
In figure \ref{pic:tbtree} the verb \emph{`bli'} is
followed by a predicative 
adjective modifying the subject tagged with \verb|SP|. Our GF grammar admits
\emph{`bli'} to have type \verb|VA|, a verb with an adjective as complement,
and the translator can hence apply the GF function \verb|ComplVA| to combine the verb
with its object. \\

\noindent The translation covers complex noun phrases which may consist of pronouns,
...
fundamental structures. If the program is extended and adapted to cover complex
...
\noindent When evaluating the mapping, the results strongly depend on which restrictions we
put on the input. 
...
Since there is no information in the grammar or lexicon about which verbs that
allows the infinitive mark to be left out, it will also allow the more
questionable sentence (\ref{sent:complVV2}b).
\eenumsentence{
\item \shortex{5}
...
when generating language, and the grammar then generates text of good
grammatical quality. \\
%ComplBareVV, SupCl
...
\noindent When it comes to parsing, we do not get far without robustness.
...
during this project.
The first grammar could parse less than half of the sentences, the result for the final
grammar was 66 \%. 
% try NPs?
...
\noindent By the renewed import of Saldo, we have doubled the size of the lexicon and thereby
...
ellipses and long and complex sentences, names and numbers,
compound words and other unknown constructions.

\subsubsection{Parallel parsing/Chunk parsing}
Instead of parsing a whole sentence in one go, we intend
parse parts of it in parallel and then try to combine the pieces into
a parse tree for the whole sentence.
...
This is also a source of information helping us separate between verbs that can
be used without an infinitive marker from the ones that cannot, cf. (\ref{sent:complVV2}a-b).
However, Lexin is relatively small and does not contain more than 10 000 entries.
%Furthermore, we also need be algorithms for
...
\noindent All parts of the project are open-source and may thus be used in other applications.
