...
% more intro to gf? Good starting sentence.
...
and GF has succesfully been used in project like %Some examples of projects using GF are 
...
There are also a number of grammar-based parsers, although none is freely avaible.
...
differences between Swedish and Norweigan. Further, there is the BiTSE 
...
The Swedish verison of the Core Language Enigne (CLE) \cite{gamback} % \cite{cle}
...
As Saldo is continously updated, the importing process has been designed to be fast
...
%\verb|UseV2| concatenate the strings ``äter" and ``råttan".
...
Swedish and Norwegian have two ways of forming future tense. The standard
form 'ska' is included in the resources as the translation to
'will'. The second tense 'kommer att' is also very commonly used, and
desereved to be covered by our grammar.
Komma is used when we believe that we are sure that it will happen:
'det kommer bli mörkt snart'
and ska more of planning
'jag ska äta mat nu'.
since we believe it will happend, it can not be as in :
'det ska (kommer*) bli fint väder imorn, men de kanske har fel'.

Since two out of the three Scandinavian languages share this tense, it was 
added to the Scandiniavin extra module.
"jeg kommer til å savne deg"

A table of result can be overview in figure \ref{fig:kommeratt}
\begin{figure}[h]
\begin{verbatim}
...
\end{verbatim}
\caption{The covered and accepted usage of future modal verb 'komma'}
\label{gfSwe:parsetree}
\end{figure}
...
the possessive pronouns as described in p. \pageref{swe:refl}.
...
He is taller than \sc{self's} friend.} \label{ex:ReflAP}
\vspace{-2mm}
\enumsentence{Han är här oftare än sin kompis.\\
He is here more often than \sc{self's} friend.} \label{ex:ReflAdv}
\vspace{-2mm}
\enumsentence{Hon tyckte om skolan och alla sina elever.\\
She liked the school and all \sc{self's} students.}\label{ex:ReflConj}
\enumsentence{Han såg sina få böcker och sin penna.\\
He saw \sc{self's}his few books and \sc{self's} pencil.}\label{ex:ReflConj2}
\vspace{-2mm}
...
He wants me to shave \sc{self's}.}
...
sub-testsuites. This way we make sure that we doesn't decrease the coverage and will
notice if the grammar starts to overgenerate. Also have some sentences that should not
be parsed. 
...
\label{sec:results}
\section{Evaluation}
The grammar in it self is not at all robust, and one unknown word will cause a whole sentence to 
fail. When parsing text, we get low results, for this reason. Not yet interesting to talk about
coverage, but focus on quality and ability to scale up. Our goal standarn is so far Talbanken,
but considering that the low coverage of the grammar without statisticts, a comparison would not be
describing anything, to little indata.
Instead we have the luck to have an expert and her presence has increased the standard.
