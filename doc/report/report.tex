...
\title{Title of the report} %A nice titleScaling up a GF grammar and preparing it for cool stuff for Swedish}
...
Hence Saldo's morpological lexicon, provided under LPGL in XML format,
...
For our purpose, only the morphological lexcion was needed.
...
flic\textbf{kor}    &    hjärt\textbf{an}  \\
flic\textbf{korna}  &    hjärt\textbf{ana} \\
\hline
\end{tabular}
\end{wrapfigure}
...
and only requiring that GF generatens one variant whenever alternatives are given. 
...
Special characters like \emph{å,ä,ö} should not occure in GF function names.
...
need to be handeld differently, usually as idioms. Pronouns(\emph{pn}) are not  
...
\hline
 ...
 Reflexive verbs  &\textbf{vbm}& \textbf{V2} & raka sig\\
 Verbs with particles &\textbf{vbm}& \textbf{V}  & hitta på\\
\end{tabular}\\
...
The same problem occured for verbs that always are used in passive form:
...
although this happend seldom. % frysa,fisa passive preterium 
...
\subsection{Parallell parsing/Chunk parsing}
\subsection{Lexicon}
%Speed up!
