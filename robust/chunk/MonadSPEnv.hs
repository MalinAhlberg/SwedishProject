...
type Grammar m t e = t -> PGF -> Morpho -> [Tree t] -> [e] -> m e
...
grammar :: (MonadWriter [String] m,MonadState s m,Ord t,Show t,Show e) 
        => ([e] -> e) -> (PGF -> t -> [Tree t] -> m ([e],[e])) -> [Rule m t e] -> Grammar m t e
grammar def parser rules = gr 
  where
    gr = \tag -> do
      
      let retry = undefined {-- \pgf m ts env -> case ts of 
            [Node w []] -> return (def [])
            ts          -> def `liftM` sequence [gr tag pgf m ts | Node tag ts <- ts] -}
      let getDeeper f = undefined {-\pgf m ts env -> do
            stored <- get 
            r <- unP f gr pgf m ts env
            case r of
              Just (e,[],_) -> return e
              Just (e,xs,_) -> tell ["Rest parse"] >> retry pgf m ts -- use xs here?
              Nothing       -> put stored          >> retry pgf m ts-}

 
      case Map.lookup tag pmap of
        Just f -> \pgf m ts env -> do 
           (parsed,env') <- parser pgf tag ts
           if null parsed then getDeeper f pgf m ts (env'++env)
...
newtype P t e m a = P {unP :: Grammar m t e -> PGF -> Morpho -> [Tree t] -> [e] -> m (Maybe (a,[Tree t],[e]))} 

instance Monad m => Monad (P t e m) where
  return x = P $ \gr pgf m ts env -> return (Just (x,ts,env))
  f >>= g  = P $ \gr pgf m ts env -> unP f gr pgf m ts env >>= \r -> case r of
                                  Just (x,ts',env') -> unP (g x) gr pgf m ts' (env'++env)
                                  Nothing           -> return Nothing

instance MonadState s m => MonadPlus (P t e m) where
  mzero     = P $ \gr pgf m ts env -> return Nothing
  mplus f g = P $ \gr pgf m ts env -> do 
    store <- get
    res <- unP f gr pgf m ts env
    case res of
...
lemmas f cat0 an0 = P $ \gr pgf morpho ts -> do
